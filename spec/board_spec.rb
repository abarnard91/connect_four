...
describe Board do
  ...
  describe "diagonal win" do
    it "sorts the array of moves and checks if there is an incremental move of x+1,y+1 or x+1,y-1 through [x+4,y+4] or [x+4,y-4]" do
      ...
      expect(game_board.diagonal_win(game_board.computer_moves_arr)).to eql(true)
      
    end
  end

  describe "check for draw" do
    it "checks on if a board if full and ends the game if the board is full with no horizontal, diagonal, or vertial win" do
      game_board = Board.new
      player = Player.new
      ...
      5.times do
        7.times do |x|
          x+= 1
          game_board.drop_chip(x,player.chip)
        end
      end
      expect(game_board.check_for_draw).to eql(false)

      1.times do 
        6.times do |y|
          y += 1 
          game_board.drop_chip(y, player.chip)
        end
      end
      expect(game_board.check_for_draw).to eql(false)

      game_board.drop_chip(7, player.chip)
      expect(game_board.check_for_draw).to eql(true)
    end
  end
